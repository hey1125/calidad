function BankPromotionsController() {
    this.Api = "FinancialEntityPromotion";
    this.ctrlActions = new ControlActions();
    this.financialEntityId = null;

    this.InitView = function () {
        // Leer financialEntityId desde querystring (?financialEntityId=123) o del último segmento
        const qs = new URLSearchParams(window.location.search);
        const qsId = parseInt(qs.get('bankId'), 10);
        const segId = (function () {
            const parts = window.location.pathname.split('/').filter(Boolean);
            const last = parseInt(parts[parts.length - 1], 10);
            return Number.isFinite(last) ? last : null;
        })();
        this.financialEntityId = Number.isFinite(qsId) && qsId > 0 ? qsId : (Number.isFinite(segId) ? segId : null);

        if (!this.financialEntityId) {
            Swal.fire({ icon: "error", title: "financialEntityId requerido", text: "Agrega ?financialEntityId={id} en la URL." });
            disableAll();
            return;
        }

        // Mostrar en UI
        $('#lblFinancialEntityId').text(this.financialEntityId);
    ...
    this.LoadAll = function () {
        if (!this.ensureEntityId()) return;

        const url = `${this.Api}/RetrieveAllByEntity?financialEntityId=${this.financialEntityId}`;
        this.ctrlActions.GetToApiWithCustomHeaders(
            url,
            (data) => this.RenderTable(data, null), // monto opcional (null)
    ...
    this.StartEdit = function (promoId) {
        ...
        this.ctrlActions.GetToApiWithCustomHeaders(
            ...
            (p) => {
                ...
                $('#EditingId').val(p.id);
                $('#fe-form-title').text('Editar promoción');

                $('#FePromoId').val(p.fePromoId);
    ...
    this.Save = function () {
        ...
        // Forzar financialEntityId desde URL
        dto.financialEntityId = this.financialEntityId;
    ...
    this.ensureEntityId = function () {
        if (!this.financialEntityId || this.financialEntityId <= 0) {
            Swal.fire({ icon: "error", title: "financialEntityId requerido", text: "Proporcione ?financialEntityId={id} en la URL." });
            return false;
        }
        return true;
    };
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group btn-group-sm" role="group">
                            <button class="btn btn-outline-primary" title="Editar" onclick="bankPromotionsController.StartEdit(${p.id})">
                                <i class="fa-solid fa-pen-to-square"></i>
                            </button>
                            <button class="btn btn-outline-danger" title="Eliminar" onclick="bankPromotionsController.Delete(${p.id})">
    ...
    this.ResetForm = function () {
        $('#EditingId').val('');
        $('#fe-form-title').text('Crear promoción');
    ...
    this.ReadForm = function () {
        ...
        // Validaciones
        if (!dto.fePromoId || !dto.startDate || !dto.endDate || !dto.status) {
            Swal.fire({ icon: "error", title: "Campos requeridos", text: "Completa FePromoId, fechas y estado." });
            return null;
        }
...
// Instancia global
const bankPromotionsController = new BankPromotionsController();
$(document).ready(function () {
    bankPromotionsController.InitView();
});

// SweetAlert override para parsear errores JSON (igual que tus otras páginas)
(function () {
